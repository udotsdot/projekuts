...
function show() {
  ...
  $(".modal-title").text("Tambah Mobil Sewaan");
...
function ajaxEdit(id) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $('[name="bbm"]').val(data.bbm);
      $('[name="transmisi"]').val(data.transmisi);
      $('[name="kursi"]').val(data.kursi);
      $('[name="kecepatan"]').val(data.kecepatan);

      if (data.gambar == "" || data.gambar == null) {
        $(".img-preview").html(
          '<label class="col-form-label col-form-label-sm">tidak ada gambar.</label>'
        );
      } else {
      ...
      $("#modal-form").modal("show");
      $(".modal-title").text("Edit Mobil Sewaan");
    },
    error: function (jqXHR, textStatus, errorThrown) {
      error(errorThrown);
    },
